...
class FactureLivraisonHandlers {
  final Repository repos;

  FactureLivraisonHandlers(this.repos);

  Router get router {
    final router = Router();

    router.get('/<business>/', (Request request, String business) async {
      List<FactureRestaurantModel> data = await repos.factureLivraisons.getAllData(business);
    ...
    router.get('/<id>', (Request request, String id) async {
      ...
      try {
        data = await repos.factureLivraisons.getFromId(int.parse(id));
      } catch (e) {
        print(e);
        return Response(404);
      }
    ...
    router.post('/insert-new-facture', (Request request) async {
      ...
      try {
        await repos.factureLivraisons.insertData(data);
      } catch (e) {
        print(e);
        return Response(422);
      }
    ...
    router.put('/update-facture/', (Request request) async {
      ...
      FactureRestaurantModel? data =
          await repos.factureLivraisons.getFromId(editH.id!); 
      ...
      repos.factureLivraisons.update(data);
      return Response.ok(jsonEncode(data.toJson()));
    });

    router.delete('/delete-facture/<id>', (Request request, String id) async {
      var id = request.params['id'];
      repos.factureLivraisons.deleteData(int.parse(id!));
    ...
    router.all(
      '/<ignored|.*>',
      (Request request) =>
          Response.notFound('La Page factureLivraisons n\'est pas trouvé'),
    );
