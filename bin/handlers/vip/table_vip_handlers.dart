...
class TableVipHandlers {
  final Repository repos;

  TableVipHandlers(this.repos);

  Router get router {
    final router = Router();

    router.get('/<business>/', (Request request, String business) async {
      List<TableRestaurantModel> data = await repos.tableVips.getAllData(business);
    ...
    router.get('/<id>', (Request request, String id) async {
      ...
      try {
        dataItem = await repos.tableVips.getFromId(int.parse(id));
      } catch (e) {
        print(e);
        return Response(404);
      }
    ...
    router.post('/insert-new-table', (Request request) async {
      ...
      try {
        await repos.tableVips.insertData(dataItem);
      } catch (e) {
        print(e);
        return Response(422);
      }
    ...
    router.put('/update-table/', (Request request) async {
      ...
      TableRestaurantModel? dataItem = await repos.tableVips.getFromId(editH.id!); 
      if (input['table'] != null) {
      ...
      repos.tableVips.update(dataItem);
      return Response.ok(jsonEncode(dataItem.toJson()));
    });
    ...
    router.all(
      '/<ignored|.*>',
      (Request request) => Response.notFound('La Page table n\'est pas trouvé'),
    );
