...
class TableRestaurantHandlers {
  final Repository repos;

  TableRestaurantHandlers(this.repos);

  Router get router {
    final router = Router();

    router.get('/<business>/', (Request request, String business) async {
      List<TableRestaurantModel> data = await repos.tableRests.getAllData(business);
    ...
    router.get('/<id>', (Request request, String id) async {
      ...
      try {
        dataItem = await repos.tableRests.getFromId(int.parse(id));
      } catch (e) {
        print(e);
        return Response(404);
      }
    ...
    router.post('/insert-new-table', (Request request) async {
      ...
      try {
        await repos.tableRests.insertData(dataItem);
      } catch (e) {
        print(e);
        return Response(422);
      }
    ...
    router.put('/update-table/', (Request request) async {
      ...
      TableRestaurantModel? dataItem = await repos.tableRests.getFromId(editH.id!); 
      if (input['table'] != null) {
      ...
      repos.tableRests.update(dataItem);
      return Response.ok(jsonEncode(dataItem.toJson()));
    });
