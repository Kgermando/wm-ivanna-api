...
class VenteEffectueeRestHandlers {
  final Repository repos;

  VenteEffectueeRestHandlers(this.repos);

  Router get router {
    final router = Router();

    router.get('/<business>/', (Request request, String business) async {
      List<VenteRestaurantModel> data = await repos.venteEffectueeRests.getAllData(business);
    ...
    router.get('/<id>', (Request request, String id) async {
      ...
      try {
        data = await repos.venteEffectueeRests.getFromId(int.parse(id));
      } catch (e) {
        print(e);
        return Response(404);
      }
    ...
    router.get('/vente-chart/<business>', (Request request, String business) async {
      List<VenteChartRestaurantModel> data = await repos.venteEffectueeRests.getAllDataChart(business);
      return Response.ok(jsonEncode(data));
    });

    router.get('/vente-chart-day/<business>', (Request request, String business) async {
      List<CourbeVenteRestaurantModel> data = await repos.venteEffectueeRests.getAllDataChartDay(business);
      return Response.ok(jsonEncode(data));
    });

    router.get('/vente-chart-month/<business>', (Request request, String business) async {
      List<CourbeVenteRestaurantModel> data = await repos.venteEffectueeRests.getAllDataChartMounth(business);
      return Response.ok(jsonEncode(data));
    });

    router.get('/vente-chart-year/<business>', (Request request, String business) async {
      List<CourbeVenteRestaurantModel> data = await repos.venteEffectueeRests.getAllDataChartYear(business);
      return Response.ok(jsonEncode(data));
    });
    ...
    router.put('/update-vente/', (Request request) async {
      ...
      VenteRestaurantModel? data =
          await repos.venteEffectueeRests.getFromId(editH.id!); 
    ...
    router.delete('/delete-vente/<id>', (Request request, String id) async {
      ...
      repos.venteEffectueeRests.deleteData(int.parse(id!));
    ...
    router.all(
      '/<ignored|.*>',
      (Request request) =>
          Response.notFound('La Page venteEffectuee Rests n\'est pas trouvé'),
    );
