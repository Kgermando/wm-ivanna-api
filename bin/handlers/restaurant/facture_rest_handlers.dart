...
class FactureRestHandlers {
  final Repository repos;

  FactureRestHandlers(this.repos);

  Router get router {
    final router = Router();

    router.get('/<business>/', (Request request, String business) async {
      List<FactureRestaurantModel> data = await repos.factureRests.getAllData(business);
    ...
    router.get('/<id>', (Request request, String id) async {
      ...
      try {
        data = await repos.factureRests.getFromId(int.parse(id));
      } catch (e) {
        print(e);
        return Response(404);
      }
    ...
    router.post('/insert-new-facture', (Request request) async {
      ...
      try {
        await repos.factureRests.insertData(data);
      } catch (e) {
        print(e);
        return Response(422);
      }
    ...
    router.put('/update-facture/', (Request request) async {
      ...
      FactureRestaurantModel? data =
          await repos.factureRests.getFromId(editH.id!); 
      ...
      repos.factureRests.update(data);
      return Response.ok(jsonEncode(data.toJson()));
    });

    router.delete('/delete-facture/<id>', (Request request, String id) async {
      var id = request.params['id'];
      repos.factureRests.deleteData(int.parse(id!));
    ...
    router.all(
      '/<ignored|.*>',
      (Request request) =>
          Response.notFound('La Page factureRest n\'est pas trouvé'),
    );
