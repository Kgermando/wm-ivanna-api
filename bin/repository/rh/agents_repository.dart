...
class AgentsRepository {
  ...
  Future<List<AgentModel>> getAllData(String business) async {
    ...
    var querySQL =
        "SELECT * FROM $tableName WHERE \"business\"='$business' ORDER BY \"created_at\" DESC;";
  ...
  Future<AgentCountModel> getCount(String business) async {
    try {
      ...
      var querySQL =
          "SELECT COUNT(*) FROM $tableName WHERE \"business\"='$business' ;";
      List<List<dynamic>> results = await executor.query(querySQL);
      for (var row in results) {
  ...
  Future<void> update(AgentModel agentModel) async {
    await executor.execute("""UPDATE $tableName
        ...
        sync = @28, async = @29 WHERE id = @30""", substitutionValues: {
      '1': agentModel.nom,
      '2': agentModel.postNom,
      '3': agentModel.prenom,
      '4': agentModel.email,
