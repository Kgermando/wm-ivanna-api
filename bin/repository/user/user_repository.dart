...
class UserRepository {
  ...
  Future<bool> isUniqueLogin(String business, String matricule) async {
    var data = await executor.query(
        "SELECT * FROM  $tableName WHERE \"business\"='$business' AND \"matricule\" = '$matricule'");
    return data.isEmpty ? true : data[0].isEmpty;
  }

  Future<int> getIdFromLoginPassword(String business,
      String matricule, String passwordHash) async {
    var data = await executor.query(
      "SELECT id FROM $tableName WHERE \"business\"='$business' AND \"matricule\"='$matricule' AND \"password_hash\"='$passwordHash'");
    return data[0][0];
  }

  Future<List<UserModel>> getAllData(String business) async {
    var data = <UserModel>{};
